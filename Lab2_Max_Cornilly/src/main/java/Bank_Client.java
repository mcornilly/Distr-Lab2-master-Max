...
public class Bank_Client {
    ...
    static int getBalance(String user){
        int balance = 0;
        try {
            balance = Integer.parseInt((String)REST_Client.GET("http://localhost:8081/bank/users/" + user));
    ...
    static int putBalance(String user, String body ) {
        int balance = 0;
        try {
            balance = Integer.parseInt((String)REST_Client.PUT("http://localhost:8081/bank/users/" + user, body));
    ...
    static long newUser(String user, String body){
        try{
            return Long.parseLong((String)REST_Client.POST("http://localhost:8081/bank/users/" + user,body));
        } catch (IOException | InterruptedException exception) {
            exception.printStackTrace();
            return 0;
        }
    ...
    static void deleteUser(String user){
        try {
            REST_Client.DELETE("http://localhost:8081/bank/users/" + user);
        } catch (IOException | InterruptedException exception) {
            exception.printStackTrace();
        }
    }
    ...
    public static void main(String[] args){
        ...
        System.out.println("GET Magalie balance: " + Bank_Client.getBalance("Magalie"));
        System.out.println("GET Bavo balance: " + Bank_Client.getBalance("Bavo"));
